...
public class FileUploadService(ILogger<FileUploadService> logger) : IFileUploadService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (section == null)
            {
                logger.LogWarning("No file section found in multipart request");
            ...
            {
                logger.LogWarning("Invalid Content-Disposition header");
            ...
            {
                logger.LogWarning("No filename provided in Content-Disposition");
                return Result<string>.Failure("File name is required");
            }

            logger.LogInformation("Processing uploaded file: {FileName}", fileName);

            // Step 3: Validate file extension
            if (!fileName.EndsWith(AllowedExtension, StringComparison.OrdinalIgnoreCase))
            {
                logger.LogWarning("Invalid file extension: {FileName}", fileName);
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                if (totalBytes == 0)
                {
                    logger.LogWarning("Uploaded file is empty");
                    return Result<string>.Failure("File was not provided or is empty.");
                }

                if (totalBytes > MaxFileSize)
                {
                    logger.LogWarning("Uploaded file exceeds maximum size: {FileSize} > {MaxSize}", totalBytes, MaxFileSize);
            ...
            logger.LogInformation("Successfully read file: {FileName} ({FileSize} bytes)", fileName, totalBytes);
            return Result<string>.Success(fileContent.ToString());
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unexpected error during multipart file reading");
